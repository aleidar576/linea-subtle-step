...
import { settingsApi, type APISetting } from '@/services/api';
...
export interface PacoteComentarioV2 {
  nome: string;
  comentarios: ComentarioItem[];
}

const SETTINGS_KEY = 'comentarios_fake_pacotes';

function gerarExemplos(): ComentarioItem[] {
  return [
    { nome: 'Maria S.', texto: 'Adorei o produto! Qualidade excelente e chegou antes do prazo. Super recomendo!', nota: 5, data: '2025-01-15', foto_avaliador: '', imagens: [] },
    { nome: 'João P.', texto: 'Produto muito bom, atendeu todas as expectativas. A embalagem veio perfeita.', nota: 4.5, data: '2025-01-10', foto_avaliador: '', imagens: [] },
    { nome: 'Ana L.', texto: 'Excelente custo-benefício! Já é minha segunda compra e não me arrependo.', nota: 5, data: '2025-01-05', foto_avaliador: '', imagens: [] },
  ];
}

type ViewMode = 'list' | 'editor';

const AdminPacotesComentarios = () => {
        ...
        try {
          ...
          if (Array.isArray(parsed)) {
            ...
            const migrated = parsed.map((p: any) => {
              ...
              // Old format: { nome, textos: string[] }
              return {
                nome: p.nome,
                comentarios: (p.textos || []).map((t: string) => ({ nome: 'Cliente', texto: t, nota: 5, data: '', foto_avaliador: '', imagens: [] })),
              };
            });
            setPacotes(migrated);
          }
  ...
  const openEdit = (idx: number) => {
    setEditIdx(idx);
    setEditNome(pacotes[idx].nome);
    setEditComentarios(JSON.parse(JSON.stringify(pacotes[idx].comentarios)));
    setMode('editor');
  };
  ...
  const handleSaveEditor = () => {
    ...
    if (editIdx === -1) {
      newPacotes.push(pacote);
    } else {
      newPacotes[editIdx] = pacote;
    }
  ...
  const handleDelete = (idx: number) => {
    savePacotes(pacotes.filter((_, i) => i !== idx));
  };
  ...
  const handleExportOne = (idx: number) => {
    ...
    a.href = url; a.download = `pacote-${p.nome}.json`; a.click();
  ...
  const handleExportEditor = () => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `pacote-${editNome || 'novo'}.json`; a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const handleImport = () => {
    try {
      ...
      const valid = arr.filter((p: any) => p.nome && (Array.isArray(p.comentarios) || Array.isArray(p.textos)));
      if (!valid.length) { toast({ title: 'JSON inválido', variant: 'destructive' }); return; }
      const migrated = valid.map((p: any) => {
        if (p.comentarios) return p;
        return { nome: p.nome, comentarios: (p.textos || []).map((t: string) => ({ nome: 'Cliente', texto: t, nota: 5, data: '', foto_avaliador: '', imagens: [] })) };
      });
      savePacotes([...pacotes, ...migrated]);
      setImportOpen(false);
      setImportJson('');
    } catch {
  ...
  const handleImportEditorConfirm = () => {
    try {
      ...
      if (data.comentarios && Array.isArray(data.comentarios)) {
        setEditComentarios(prev => [...prev, ...data.comentarios]);
      } else if (Array.isArray(data)) {
        setEditComentarios(prev => [...prev, ...data]);
      }
      setImportOpen(false);
      setImportJson('');
  ...
  const addComentario = () => {
    setEditComentarios(prev => [...prev, { nome: '', texto: '', nota: 5, data: new Date().toISOString().split('T')[0], foto_avaliador: '', imagens: [] }]);
  };

  const updateComentario = (idx: number, key: keyof ComentarioItem, value: any) => {
    setEditComentarios(prev => {
      const list = [...prev];
  ...
  const removeComentario = (idx: number) => {
    setEditComentarios(prev => prev.filter((_, i) => i !== idx));
  };
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" className="gap-1" onClick={handleImportEditor}>
              <Upload className="h-4 w-4" /> Importar JSON
            </Button>
            <Button variant="outline" size="sm" className="gap-1" onClick={handleExportEditor}>
              <Download className="h-4 w-4" /> Exportar JSON
            </Button>
            <Button variant="outline" size="sm" className="gap-1" onClick={() => setEditComentarios(gerarExemplos())}>
              <Wand2 className="h-4 w-4" /> Gerar Exemplo
            </Button>
            <Button onClick={handleSaveEditor} disabled={saving} className="gap-1">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input value={editNome} onChange={e => setEditNome(e.target.value)} placeholder="Ex: Avaliações Positivas" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {editComentarios.map((c, i) => (
              <div key={i} className="border border-border rounded-xl p-4 space-y-3 bg-card">
                ...
                <div className="flex items-center gap-3">
                  {c.foto_avaliador ? (
                    <img src={c.foto_avaliador} alt={c.nome} className="h-10 w-10 rounded-full object-cover border border-border shrink-0" />
                  ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <ImageUploader
                      lojaId="admin"
                      value={c.foto_avaliador || ''}
                      onChange={(url) => updateComentario(i, 'foto_avaliador', url)}
                ...
                <div className="flex gap-0.5">
                  {[1, 2, 3, 4, 5].map(s => (
                    <Star key={s} className={`h-3.5 w-3.5 ${s <= Math.floor(c.nota) ? 'fill-primary text-primary' : 'text-muted-foreground/30'}`} />
                  ))}
                  <span className="text-xs text-muted-foreground ml-1">{Number(c.nota).toFixed(1)}</span>
                </div>
                ...
                <div className="space-y-1">
                  <Label className="text-xs">Fotos do comentário (até 3)</Label>
                      ...
                      <div key={j} className="relative group">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            imgs.splice(j, 1);
                            updateComentario(i, 'imagens', imgs);
                          }}
        ...
        {/* Import Dialog (editor mode) */}
        <Dialog open={importOpen} onOpenChange={setImportOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader><DialogTitle>Importar Comentários via JSON</DialogTitle></DialogHeader>
            <Textarea
              className="min-h-[200px] font-mono text-sm"
              value={importJson}
              onChange={e => setImportJson(e.target.value)}
              placeholder='[{"nome": "Maria", "texto": "Adorei!", "nota": 5, "data": "2025-01-01"}]'
            />
            <DialogFooter>
              <Button variant="outline" onClick={() => setImportOpen(false)}>Cancelar</Button>
              <Button onClick={handleImportEditorConfirm}>Importar</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Pacotes de Comentários</h1>
        <div className="flex gap-2">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Pacotes criados aqui ficam disponíveis para <strong>todas as lojas</strong> como "Comentários Genéricos".
      </p>

      {pacotes.length === 0 ? (
        <div className="bg-card border border-border rounded-xl p-12 text-center">
          <FileJson className="h-12 w-12 text-muted-foreground mx-auto mb-3" />
          <p className="font-medium mb-1">Nenhum pacote de comentários</p>
          <p className="text-sm text-muted-foreground mb-4">Crie pacotes para que os lojistas possam usar nos produtos.</p>
          ...
          {pacotes.map((p, idx) => (
            <div key={idx} className="bg-card border border-border rounded-xl p-5 space-y-3 cursor-pointer hover:border-primary/50 transition-colors" onClick={() => openEdit(idx)}>
              ...
              <div className="max-h-24 overflow-y-auto space-y-1">
                {p.comentarios.slice(0, 3).map((c, i) => (
                  <div key={i} className="flex items-center gap-2">
                    {c.foto_avaliador ? (
                      <img src={c.foto_avaliador} alt="" className="h-5 w-5 rounded-full object-cover shrink-0" />
                    ) : (
              ...
              <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                <Button variant="outline" size="sm" className="gap-1 flex-1" onClick={() => openEdit(idx)}>
                  Editar
                </Button>
                <Button variant="outline" size="sm" className="gap-1" onClick={() => handleExportOne(idx)}>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>Excluir pacote "{p.nome}"?</AlertDialogTitle>
                      <AlertDialogDescription>Esta ação não pode ser desfeita.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Cancelar</AlertDialogCancel>
                      <AlertDialogAction onClick={() => handleDelete(idx)}>Excluir</AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
                </AlertDialog>
              </div>
...
export default AdminPacotesComentarios;
