...
async function handleWebhookEvent({ event, rawBody, sig }) {
  ...
  try {
    ...
    if (event.type === 'invoice.payment_succeeded') {
      ...
      if (lojista) {
        // Não sobrescrever 'trialing' quando é a invoice inicial do trial ($0)
        if (billingReason === 'subscription_create' && lojista.subscription_status === 'trialing') {
          console.log(`[STRIPE-WEBHOOK] ℹ️ Invoice de criação durante trial — mantendo status 'trialing' para ${lojista.email}`);
        } else {
