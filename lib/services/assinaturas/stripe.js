        ...
        lojista.historico_assinatura.push({
          ...
          detalhes: billingReason === 'subscription_create'
            ...
            : billingReason === 'manual'
              ? 'Cobrança de taxas de transação processada com sucesso.'
...
async function processarCronTaxas() {
  ...
  for (const lojista of lojistas) {
    try {
      ...
      // Limite mínimo de R$ 10,00 para cobrança de taxas
      if (amountCents < 1000) {
        console.log(`[CRON-TAXAS] Lojista ${lojista.email} possui taxas de R$ ${lojista.taxas_acumuladas.toFixed(2)}, inferior ao mínimo de R$ 10,00. Adiando vencimento.`);
      ...
      if (lojista.tentativas_taxas >= 3) {
        lojista.status_taxas = 'bloqueado';
        lojista.data_bloqueio_taxas = new Date();
        lojista.historico_assinatura.push({
...
async function pagarTaxasManual({ user }) {
  ...
  // Verifica o mínimo de R$ 10,00 para cobrança manual
  if (amountCents < 1000) {
    return { error: 'O valor mínimo para pagamento de taxas é de R$ 10,00.', httpStatus: 400 };
  }
