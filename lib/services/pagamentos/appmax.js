...
async function createPayment({ amount, method, customer, shipping, items, card_token, installments, loja_id }) {
  ...
  try {
    ...
    const document_number = (customer.taxId || customer.cpf || '').replace(/\D/g, '');
    ...
    const orderPayload = {
      ...
      products: (items || []).map(item => ({
        ...
        name: item.name || 'Produto',
        quantity: item.quantity || 1,
        unit_value: item.price || 0, // centavos
      })),
    };

    // Calcular shipping_value se disponível
    if (shipping?.valor || shipping?.price) {
      orderPayload.shipping_value = shipping.valor || shipping.price || 0;
    }
