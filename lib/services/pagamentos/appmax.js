...
async function createPayment({ amount, method, customer, shipping, items, card_token, installments, loja_id }) {
  ...
  try {
      ...
      paymentPayload = {
        ...
        payment_data: {
          credit_card: {
            ...
            holder_document_number: document_number,
            soft_descriptor: (loja.nome || 'LOJA').substring(0, 13).toUpperCase(),
          },
        },
      };
    } else {
    ...
    let payResult = await appmaxApiFetch(apiUrl, paymentPath, 'POST', paymentPayload, access_token);

    // Resiliência PIX: alguns ambientes sandbox aceitam payload sem document_number no objeto pix
    if (!payResult.ok && method === 'pix') {
      const fallbackPixPayload = {
        order_id,
        customer_id,
        payment_data: { pix: {} },
      };
      console.warn('[APPMAX] PIX retornou erro, tentando fallback com pix vazio...');
      console.log('[APPMAX] PIX fallback payload:', JSON.stringify(fallbackPixPayload));
      payResult = await appmaxApiFetch(apiUrl, paymentPath, 'POST', fallbackPixPayload, access_token);
    }

    if (!payResult.ok) {
      return {
        error: payResult.data?.message || payResult.data?.error?.message || 'Erro ao processar pagamento na Appmax',
    ...
    if (method === 'pix') {
      ...
      normalized.pdf_url = boletoData.url || boletoData.pdf_url || boletoData.boleto_url || '';
      normalized.digitable_line = boletoData.digitable_line || boletoData.linha_digitavel || '';
    } else if (method === 'credit_card') {
...
async function handleWebhook({ body, req }) {
  ...
  let mappedStatus;
  if (rawEvent === 'order_approved' || rawEvent === 'order_paid' || rawEvent === 'order_paid_by_pix') {
    mappedStatus = 'pago';
