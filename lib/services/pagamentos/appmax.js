...
async function handleWebhook({ body, req }) {
  ...
  // === ACUMULAR TAXAS DA PLATAFORMA (Stripe) quando pedido é confirmado como pago ===
  if (mappedStatus === 'pago' && result.total > 0) {
    try {
      ...
      if (lojaDoc) {
        ...
        if (lojistaDoc && lojistaDoc.modo_amigo) {
          console.log(`[APPMAX-WEBHOOK] Lojista ${lojistaDoc.email} é VIP (modo_amigo) — taxa zerada`);
          ...
          if (plano) {
            ...
            console.log(`[APPMAX-WEBHOOK] Taxa plataforma acumulada: R$ ${valorTaxaArredondado.toFixed(2)} para ${lojistaDoc.email} (total: R$ ${lojistaDoc.taxas_acumuladas.toFixed(2)})`);
          }
        }
      }
    } catch (taxErr) {
      console.error('[APPMAX-WEBHOOK] Erro ao acumular taxa da plataforma:', taxErr.message);
    }
  }
