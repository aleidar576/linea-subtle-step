...
async function createPayment({ amount, method, customer, shipping, items, card_data, installments, loja_id }) {
  ...
  try {
    ...
    if (method === 'pix') {
      ...
      normalized.pdf_url = boletoData.pdf || boletoData.url || boletoData.pdf_url || boletoData.boleto_url
        || payData.pdf || payData.url || payData.pdf_url || payData.boleto_url
        || payResult.data?.pdf || payResult.data?.url || payResult.data?.boleto_url || '';
      normalized.digitable_line = boletoData.digitable_line || boletoData.linha_digitavel
        || payData.digitable_line || payData.linha_digitavel
        || payResult.data?.digitable_line || payResult.data?.linha_digitavel || '';
    } else if (method === 'credit_card') {
