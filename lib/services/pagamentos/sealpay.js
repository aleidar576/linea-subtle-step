...
async function handleWebhook({ txid, status, req }) {
  ...
  // Confirmar pagamento centralizado (marca pago + taxas + carrinho)
  const { confirmarPagamentoPedido } = require('../pedidos/confirmarPagamento.js');
  const confirmResult = await confirmarPagamentoPedido({ txid, logPrefix: '[SEALPAY-WEBHOOK]' });

  if (confirmResult.ok && confirmResult.pedido) {
    const result = confirmResult.pedido;
    ...
    console.warn(`[SEALPAY-WEBHOOK] ${confirmResult.error} (txid: ${txid})`);
  }
